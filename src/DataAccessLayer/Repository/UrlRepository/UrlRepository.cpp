...
UrlRepository::GetBy(const AliasUrlEntity& alias_url)
{
    ...
    {
        if(alias_url.IsNull())
            throw std::runtime_error("UrlRepository::GetBy() : KeyObject is null");
        ...
        if(query_result.isNil())
            throw std::runtime_error("UrlRepository::GetBy(): empty result with command: "+ alias_key );
...
UrlRepository::Create(const ComposedUrlsEntity& composed_urls)
{
    try
    {
        ...
        if(alias.IsNull() || original.IsNull())
            throw std::runtime_error("UrlRepository::Create() : ComposedObject is null");
        ...
        LOG_DEBUG <<"UrlRepository::Create() : Insertion completed\n";
        co_return;
    }
    catch(const std::exception& e)
    {
